...
import { ChevronRight, Map as MapIcon, BookOpen, Trophy, Star, ShieldCheck } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    const unsubRoadmaps = onSnapshot(query(collection(db, 'roadmaps'), orderBy('order', 'asc')), (snapshot) => {
      setRoadmaps(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Roadmap)));
    }, (error) => {
    ...
    return () => { unsubRoadmaps(); unsubArticles(); };
  }, []);
    ...
    <div className="space-y-12">
      ...
      <section className="relative py-20 overflow-hidden rounded-3xl bg-card border border-border">
        ...
        <div className="relative z-10 px-8 md:px-12 text-center md:text-left max-w-3xl">
          ...
          >
            Master Modern <span className="text-emerald-500">Development</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Follow expert-curated roadmaps, complete interactive quizzes, and earn XP as you build your career in tech.
          </motion.p>
          {!profile && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Choose a roadmap to begin your quest</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <h3 className="text-xl font-bold mb-3 group-hover:text-emerald-500 transition-colors">{roadmap.title}</h3>
            ...
            {roadmaps.length === 0 && (
              <div className="col-span-full py-20 text-center border-2 border-dashed border-border rounded-3xl space-y-6">
                ...
                <div>
                  <p className="text-muted-foreground font-medium">No roadmaps available yet.</p>
                  {(profile?.role === 'admin' || profile?.email === 'vithyaa.shankarr@gmail.com') ? (
                    <div className="mt-6">
                      <Link 
                        href="/admin" 
                      ...
                      >
                        <ShieldCheck className="w-5 h-5" /> Go to Admin Portal to Seed Data
                      </Link>
                    </div>
                  ) : (
                    <p className="text-muted-foreground text-sm">Check back soon for new learning paths!</p>
                  )}
                </div>
              </div>
            )}
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-8 bg-card border border-border rounded-3xl">
          ...
          <div className="text-muted-foreground text-sm font-medium uppercase tracking-wider">Active Quests</div>
        </div>
        <div className="p-8 bg-card border border-border rounded-3xl">
          <div className="text-3xl font-bold text-emerald-500 mb-1">500+</div>
          <div className="text-muted-foreground text-sm font-medium uppercase tracking-wider">Articles</div>
        </div>
        <div className="p-8 bg-card border border-border rounded-3xl">
          <div className="text-3xl font-bold text-emerald-500 mb-1">1M+</div>
          <div className="text-muted-foreground text-sm font-medium uppercase tracking-wider">XP Earned</div>
        </div>
      </section>
    </div>
  );
