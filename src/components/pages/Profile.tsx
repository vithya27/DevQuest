...
export default function Profile() {
  ...
  React.useEffect(() => {
    if (!profile || !redirectTo || !redirectTo.startsWith("/")) return;
    router.replace(redirectTo);
  }, [profile, redirectTo, router]);
    ...
    return (
      <div className="py-20 text-center space-y-6">
        ...
        <p className="text-muted-foreground max-w-md mx-auto">
          Join thousands of developers leveling up their skills and earning
          rewards.
        </p>
      </div>
    );
      ...
      <section className="relative p-8 md:p-12 bg-card rounded-[2.5rem] border border-border overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="grid grid-cols-2 gap-4 w-full md:w-auto">
            <Link
              href="/leaderboard"
              className="p-6 bg-background/50 rounded-3xl border border-border text-center hover:border-emerald-500/50 transition-colors group"
            >
            ...
            <div className="p-6 bg-background/50 rounded-3xl border border-border text-center">
              ...
              <div className="text-2xl font-bold font-mono">
                {profile.completedArticles.length}
              </div>
              <div className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">
                Quests
              </div>
            ...
            {[
              {
                title: "First Quest",
                icon: Star,
                unlocked: profile.completedArticles.length > 0,
              },
              {
                title: "XP Hunter",
                icon: Zap,
              ...
              },
              {
                title: "Roadmap King",
                icon: Trophy,
                unlocked: profile.completedArticles.length >= 10,
              },
            ].map((badge, i) => (
              <div
                key={i}
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {profile.completedArticles.length > 0 ? (
              profile.completedArticles
                .slice(-5)
                .reverse()
                .map((id, i) => (
                  <div
                    key={i}
                    className="p-4 bg-card rounded-2xl border border-border flex items-center justify-between group"
                  >
              ...
              <div className="p-8 text-center bg-card/30 border border-dashed border-border rounded-3xl">
                <p className="text-muted-foreground text-sm italic">
                  No activity yet. Go start a quest!
                </p>
              </div>
            )}
          </div>
